...
> 审计对象：`cursor/agent-multi-novel-games-b603` @ **`48ded19`**（全流水线基准）+ 增审至 **`eae1ddc`**（灭运 G8 改名 `c072c80` 落地后复测：412 测试 + build 复绿）
...
| 游戏 | 得分（/12） | 发布就绪 | 一句话裁定 |
|---|---|---|---|
| 烂柯 G1 | **11.5** | **✅ YES** | 工程最全面；仅剩 README 快捷键表（G7）即满分 |
| 灭运 G2 | **11.0** | **✅ YES（带注）** | 内容与防作弊最深；G8 改名已落地，剩 lint 门 + README 快捷键表 |
...
> 计分：✅=1 · ⚠️=0.5 · ❌=0，共 12 门。分数含 `c072c80` G8 复审调整（烂柯/灭运 G8 ⚠️→✅，道君得分随 G8 复核 +0.5）。
