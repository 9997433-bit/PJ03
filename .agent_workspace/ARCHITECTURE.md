...
> 作者：R1-F1（架构规划代理）；R2-F1 增补 §6 身份表勘正、§10 命名冲突登记表、§11 文档冲突裁定。
> 本文档是四款游戏（凡人/烂柯/灭运/道君）的**结构性事实来源**：目录布局、共享与独立代码边界、构建发布流水线、质量门。
...
（R2-F1 勘正：叙述者与主题以 `briefs/` 为准 —— SOTA G8 验收即对照 briefs；SAVE_MAGIC 与封印掷标记一并登记于此。）

| id | 游戏 | 叙述者 | 存档 key | MAGIC | 封印掷标记 | 主题基调 |
|---|---|---|---|---|---|---|
| `mortal` | 凡人修仙传 | 天道 | `mcls_save_v1`（既有，不动） | `MCLS` | 暗掷 | 玄墨鎏金（墨黑/金玉/朱砂，暗） |
| `lanke` | 烂柯棋缘 | 弈者 | `lanke_save_v1` | `LKQY` | 覆枰 | 宣纸浅色（米白/松烟墨/青竹/朱印红，**四作唯一浅色**） |
| `mieyun` | 灭运图录 | 司命 | `mieyun_save_v1` | `MYTL` | 匿运 | 玄青星图（深靛/银白星芒/荧金/劫赤） |
| `daojun` | 道君 | 道尊 | `daojun_save_v1` | `DJUN` | 藏机 | 紫金穹宇（玄紫/鎏紫/帝金/魂青） |
...
## 10. 跨作命名冲突登记表（R2-F1 · G8「术语零重合」整改单）

审计四作**已落盘代码**发现的撞名及裁定（对应游戏 PLAN.md 已同步）：

| 冲突 | 现状证据 | 裁定 |
|---|---|---|
| 境界名：道君 `炼气/筑基/金丹/元婴/化神` 与根游戏完全重合 | `games/dao-jun/engine/types.ts` REALMS | G3 改为道纹主题七阶：观纹→铭纹→织络→凝魂→御土→合道→道君 |
| 境界名：烂柯 `凡尘`/`通玄` 与灭运 `凡尘`/`通玄` 互撞 | 两作 `data/realms.ts` | 烂柯保留（R1 数据更完整）；灭运 `mortal`→**未录**（契合其「星盘未录」设定）、`tongxuan`→**窥命** |
| 货币显示名：灭运/道君代码均用「灵石」，与根游戏重合 | 字段 `stones`/`spiritStones` | 字段名不动，显示名改：灭运→**玄晶**，道君→**玄玉**（brief 同）；烂柯用银钱/文钱，无冲突 |
| 市场名：烂柯代码注释沿用根游戏「坊市」 | G1 `types.ts` market 注释 | 烂柯→**墟市**、灭运→**万法坊**、道君→**法会**（均已入各 PLAN 指令表） |
| 叙述者：烂柯 `Speaker` 联合类型残留 `'天道'` | G1 `engine/types.ts` | 移除，统一 `'弈者' \| '棋录' \| '汝'` |
| 战斗战术名：灭运 brief `服丹` 与道君草案重复 | briefs | 灭运保留 `服丹`；道君战斗六式定为 力破/周旋/布纹/摄神/吞丹/遁土 |

新增术语一律先查此表 + 三份 briefs，再入 data。dataIntegrity 建议各作加一条「禁用他作术语字样」回归断言（G3 道君改名后尤其要防「炼气/筑基…」残留）。

## 11. 文档冲突裁定（R2-F1 · 并发交付去重）

R1-F1（从零设计）与 R2-F1（依已落盘代码盘点）先后交付了同路径的三份 `games/*/PLAN.md`。裁定如下：

- **现行 PLAN.md = R2-F1 代码接地版**。理由：R2 简报要求「补齐」G1/G2 半成品而非重写；实现代理正在完成的正是 R1 落盘的领域模型（烂柯五心/棋缘/七境，灭运五维/气运劫运/劫击表，道君纹魂疆域三轨）。与实现脱节的计划会把 R2 实现代理引向推倒重来。
- R1-F1 从零设计稿保留在 git 历史（`0ce2528` / `8dde51c` / `64daffc`），其中可移植的创意（烂柯护身缘、灭运命轨快照入审计、道君斗法/厮杀两态）已在各 PLAN 的候补清单中引用或可在 R3 择优吸收。
- **briefs/ 是创意与主题事实源**（G8 验收对照物）；PLAN 与 brief 冲突处，机制骨架从代码、叙述者与视觉从 brief（已按此原则写入三份 PLAN 开头的裁定条款）。
- 本文档以 R1-F1 版为基底，由 R2-F1 增补而非替换 —— 避免重演 `CONFLICT-lanke-qiyuan-O1.md` 记录的互相覆盖。后续轮次修订文档请**编辑现行版**，不要另起炉灶。
