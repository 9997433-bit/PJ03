/**
 * save.ts — versioned save/load with checksum & migration.
 *
 * Adapted from the root simulator. The engine never touches `localStorage`
 * directly: persistence goes through an injected `StorageAdapter`.
 * `getBrowserStorage()` supplies the real adapter in the browser;
 * `createMemoryStorage()` serves tests and SSR.
 *
 * A save blob is a `SaveEnvelope` wrapping the GameState payload with a
 * schema version, a SHA-256 checksum and a copy of the payload's auditHash.
 * Any mismatch on load ⇒ 「此局棋谱残损,不可续弈。」 — offer 重开 only.
 */
 ...
 * version n+1. Empty while the schema is at v1.
 */
...
export function deserializeSave<T extends SaveableState>(
  ...
  if (raw === null || raw.trim() === '') {
    return { ok: false, code: 'empty', message: '此间尚无棋局。' };
  }
  ...
  if (parsed.version > CURRENT_SAVE_VERSION) {
    return {
      ok: false,
      code: 'version',
      message: `此谱来自将来之局 (v${parsed.version} > v${CURRENT_SAVE_VERSION})。`,
    };
  }
  ...
  while (version < CURRENT_SAVE_VERSION) {
    ...
    if (!step) {
      return {
        ok: false,
        code: 'version',
        message: `无法自 v${version} 迁至 v${CURRENT_SAVE_VERSION}。`,
      };
    }
